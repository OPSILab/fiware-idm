...
function process_environment_variables(verbose) {
    ...
    if (process.env.IDM_CORS_ENABLED) {
        ...
        if (process.env.IDM_CORS_ORIGIN) {
            config.cors.options.origin = process.env.IDM_CORS_ORIGIN == 'true' ? true : (to_array(process.env.IDM_CORS_ORIGIN, '*'));
        }
        if (process.env.IDM_CORS_METHODS) {
            config.cors.options.methods = to_array(process.env.IDM_CORS_METHODS, ['GET', 'HEAD', 'PUT', 'PATCH', 'POST', 'DELETE']);
        }

        if (process.env.IDM_CORS_ALLOWED_HEADERS) {
            config.cors.options.allowedHeaders = process.env.IDM_CORS_ALLOWED_HEADERS;
        }
        if (process.env.IDM_CORS_EXPOSED_HEADERS) {
            config.cors.options.exposedHeaders = process.env.IDM_CORS_EXPOSED_HEADERS;
        }
        if (process.env.IDM_CORS_CREDENTIALS) {
            config.cors.options.credentials = process.env.IDM_CORS_CREDENTIALS;
        }
        if (process.env.IDM_CORS_MAX_AGE) {
            config.cors.options.maxAge = process.env.IDM_CORS_MAX_AGE;
        }
        if (process.env.IDM_CORS_PREFLIGHT) {
            config.cors.options.preflightContinue = process.env.IDM_CORS_PREFLIGHT;
        }
        if (process.env.IDM_CORS_OPTIONS_STATUS) {
            config.cors.options.optionsSuccessStatus = process.env.IDM_CORS_OPTIONS_STATUS;
        }
    }
    ...
    // Config SPID Authentication
    config.spid = config.spid || {};
    if (process.env.IDM_SPID_ENABLED) {
        config.spid.enabled = to_boolean(process.env.IDM_SPID_ENABLED, false);

        if (process.env.IDM_SPID_GATEWAY_HOST) {
            config.spid.gateway_host = process.env.IDM_SPID_GATEWAY_HOST;
        }
        if (process.env.IDM_SPID_NODE_HOST) {
            config.spid.node_host = process.env.IDM_SPID_NODE_HOST;
        }
        if (process.env.IDM_SPID_METADATA_LIFETIME) {
            config.spid.metadata_expiration = process.env.IDM_SPID_METADATA_LIFETIME;
        }
    }

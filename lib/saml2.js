...
create_authn_request = function (
    ...
    if (saml_type === 'spid') {
        body["saml2p:AuthnRequest"]['@AttributeConsumingServiceIndex'] = '0';
        body["saml2p:AuthnRequest"]['saml2:Issuer']['@NameQualifier'] = issuer;
    } else { // eidas
        body["saml2p:AuthnRequest"]['saml2p:Extensions'] = extensions;
        body["saml2p:AuthnRequest"]['@ProviderName'] = provider_name;
        body["saml2p:AuthnRequest"]['@Consent'] = 'urn:oasis:names:tc:SAML:2.0:consent:unspecified';
        body["saml2p:AuthnRequest"]['@IsPassive'] = 'false';
        body["saml2p:AuthnRequest"]['saml2p:NameIDPolicy']['@AllowCreate'] = 'true';
    }
...
create_metadata = function (
    ...
    let attribute_consumingservice_descriptor = (function () {
        let requestedAttributes = [];
        for (const attribute of Object.keys(attributes_list)) {
        ...
        return {
            'md:AttributeConsumingService': []
                .concat({
                    '@index': '0',
                    'md:ServiceName': { '@xml:lang': 'it', '#text': 'Set 0' }
                }).concat(requestedAttributes)
        };
    })();
        ...
        'md:EntityDescriptor': []
            .concat({
                ...
                'md:SPSSODescriptor': []
                    ...
                    ]).concat(attribute_consumingservice_descriptor)
            })
            .concat(person1)
            .concat(person2)
    };
    ...
    if (saml_type === 'eidas')
        ...
        organization_obj = {
            'md:OrganizationName': {
                '@xml:lang': 'it', '#text': organization.name
            },
            'md:OrganizationDisplayName': {
                '@xml:lang': 'it', '#text': organization.name
            },
            'md:OrganizationURL': {
                '@xml:lang': 'it', '#text': organization.url
            }
        };

    body['md:EntityDescriptor'][0]['md:Organization'] = organization_obj;
...
sign_metadata = function (xml, private_key, options, certificate) {
    ...
    signer.signatureAlgorithm = 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha512';
    signer.keyInfoProvider = new MyKeyInfo(certificate);
    signer.computeSignature(xml, { prefix: 'ds', location: { reference: 'md:SPSSODescriptor', action: 'before' } });
    return signer.getSignedXml();
...
parse_assertion_attributes = function (dom, saml_type) {
    let assertion;
    ...
    for (j = 0, len = ref1.length; j < len; j++) {
        attribute = ref1[j];
        attribute_name = saml_type === 'spid' ? get_attribute_value(attribute, 'Name'): get_attribute_value(attribute, 'FriendlyName');
    ...
    return async.waterfall(
        [
            function (cb_wf) {
                ...
                let signed_response_data;

                let signed_assertion_data = {};
                ...
                try {
                    signed_response_data = check_saml_signature(saml_response_str, idp_certificate);
                } catch (error) {
                    ex = error;
                    return cb_wf(
                        new Error(
                            'SAML Response signature check failed!' + ex.message
                        )
                    );
                }
                if (!signed_response_data)
                    return cb_wf(
                        new Error('SAML Response signature check failed!')
                    );


                // If Spid SAML case, do also signature check of assertion part
                if (saml_type === 'spid') {
                    try {

                        const assertion = xpath.select(
                            "//*[local-name(.)='Assertion']",
                            xml
                        );
                        if (!assertion || assertion.length == 0)
                            throw new Error("No Assertion Signature found on SAML Response");
                        signed_assertion_data = check_saml_signature(assertion[0].toString(), idp_certificate);

                    } catch (error) {
                        ex = error;
                        return cb_wf(
                            new Error('SAML Assertion signature check failed!' + ex.message)
                        );
                    }

                    if (!signed_assertion_data)
            ...
            function (encrypted_assertion, cb_wf) {
                ...
                const decrypt_callback = function (err, result) {
                    let assertion;
                ...
                if (saml_type === 'spid')
                    // assertion is not encrypted in SPID case
                    return decrypt_callback(null, encrypted_assertion.toString());
                else
                    return decrypt_assertion(encrypted_assertion, sp_private_keys, decrypt_callback);
            },
            function (decrypted_assertion, cb_wf) {
                ...
                try {
                  ...
                  if (session_info.not_on_or_after != null) {
                    ...
                    assertion_attributes = parse_assertion_attributes(decrypted_assertion, saml_type);
...
module.exports.ServiceProvider = ServiceProvider = (function () {
    function ServiceProvider(options) {
        ...
        this.attributes_list = options.attributes_list;
        this.saml_type = options.saml_type;
